...
SX1278Task::SX1278Task(QueueHandle_t& _RXQ, QueueHandle_t& _TXQ, uint32_t _cs, uint32_t _irq, uint32_t _rst,
                       uint32_t _gpio, bool enableRX, bool enableTX, uint8_t _syncWord)
    : cs(_cs),
      irq(_irq),
      rst(_rst),
      gpio(_gpio),
      LoRaTask(_RXQ, _TXQ, enableRX, enableTX, 1000),  // TXDelay milli sec.
      syncWord(_syncWord) {}
